...
```sql
-- Dodaje order jeśli ktoś płaci od razu
...
```sql
-- Dodaje order jeśli ktoś płaci miesięcznie
...
```sql
-- pokazuje pracowników którzy są przypisani do danego Zamówienia
...
CREATE FUNCTION show_taken_tables_from_x_to_y_with_z_chairs(
    @OrderCompletionStart datetime,
    @OrderCompletionEnd datetime,
    @ChairNeeded int
)
    RETURNS TABLE
        AS
        RETURN
        SELECT T.TableID, O.OrderID, O.CustomerID, O.OrderCompletionDate, O.DurationTime, T.ChairAmount
        FROM [Order] O
                 JOIN [Reservation] R2 on O.OrderID = R2.OrderID
                 JOIN [Table] T on R2.TableID = T.TableID
        WHERE O.OrderCompletionDate >= @OrderCompletionStart
          AND DATEADD(SECOND, DATEDIFF(SECOND, '00:00:00', O.DurationTime), O.OrderCompletionDate) <=
              @OrderCompletionEnd
          AND T.ChairAmount >= @ChairNeeded
...
CREATE FUNCTION show_free_tables_from_x_to_y_with_z_chairs(
    ...
    RETURNS TABLE
        AS
        RETURN
        SELECT T.TableID, T.ChairAmount
        FROM [Table] T
        WHERE T.TableID NOT IN
              (
                  select Q.TableID
                  from dbo.show_taken_tables_from_x_to_y_with_z_chairs(@OrderCompletionStart, @OrderCompletionEnd,
                                                                       @ChairNeeded) Q
              ) AND T.isActive = 1
```
